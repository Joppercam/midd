...
<head>
    ...
    <title>Libro de Ventas - {{ $book->period_name }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width: 8%" class="text-right">Exento</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">{{ $entry->status === 'active' ? 'A' : 'N' }}</td>
                </tr>
                @endforeach
            </tbody>
            <tfoot>
        ...
        <div class="summary-table">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ \App\Models\SalesBookEntry::find(1)->getDocumentTypeLabelAttribute($item['document_type']) ?? $item['document_type'] }}</td>
        ...
        <div class="footer">
            ...
            <p><strong>IVA Débito Fiscal del período:</strong> ${{ number_format($book->total_tax, 0, ',', '.') }}</p>
            @if($book->total_credit_notes > 0)
                <p><strong>Total Notas de Crédito:</strong> ${{ number_format($book->total_credit_notes, 0, ',', '.') }}</p>
            @endif
            @if($book->total_debit_notes > 0)
                <p><strong>Total Notas de Débito:</strong> ${{ number_format($book->total_debit_notes, 0, ',', '.') }}</p>
            @endif
