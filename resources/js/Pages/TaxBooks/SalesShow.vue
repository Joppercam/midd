<template>
    <Head :title="`Libro de Ventas - ${book.period_name}`" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    Libro de Ventas - {{ book.period_name }}
                </h2>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="entry in book.entries" :key="entry.id">
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap text-center">
                                            <Badge :type="entry.status === 'active' ? 'green' : 'red'">
                                                {{ entry.status_label }}
                                            </Badge>
                                        </td>
                                    </tr>
...
const finalizeBook = () => {
    if (confirm('¿Finalizar este libro? Una vez finalizado no podrá ser modificado.')) {
        router.post(route('tax-books.sales.finalize', props.book.id));
    }
};

const exportBook = (format) => {
    const url = format === 'excel' 
        ? route('tax-books.sales.export.excel', props.book.id)
        : route('tax-books.sales.export.pdf', props.book.id);
